...
_YAML_OMIT = {
    "level", "sql_config", "api_config",
    "summarySuggestion", "keywords", "score",
}
...
def to_clean_json(tree: dict) -> dict:
    ...
    保留字段：id, name, level, description, descriptionSuggestion, condition, condition_queries,
             summarySuggestion, summary, sql_config, api_config（L5）, children
    缺失的通用字段补默认值，确保所有层级的节点结构一致。
    """
    _KEEP = {"id", "name", "level", "description", "descriptionSuggestion", "condition", "condition_queries",
             "summarySuggestion", "summary", "sql_config", "api_config"}
    node = {k: v for k, v in tree.items() if k in _KEEP}
    # 补默认值：保证任意层级的节点都有这几个字段
    node.setdefault("condition", "")
    node.setdefault("condition_queries", [])
    node.setdefault("descriptionSuggestion", "")
