...
对比模板大纲与知识图谱，识别模板中新建的节点（id 含 `new_`），逐一判断：

| 判断 | 动作 |
|------|------|
| 知识图谱中无语义相近节点，且有跨场景通用价值 | `add_node`：新增 L2/L3/L4 节点 |
| 知识图谱中已有语义相近节点，模板提供了新的业务视角 | `enrich_existing`：追加描述 |
| 节点过于场景专属，或与已有节点完全重复 | `ignore`：忽略 |

**L5（query）节点不参与融合**——query 节点绑定 SQL/API 查询，由开发人员维护，不在分析范围内。

**add_node 必填字段：**
- `level`：2、3 或 4（不允许 1 或 5）
- `name`：节点名称
- `keywords`：3～6 个检索关键词
- `description`：50～100 字的业务描述
- `parent_id`：必须是知识图谱中已有节点的 ID（如 `L2_001`）
