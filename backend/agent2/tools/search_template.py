"""
search_template.py — Tool: search_outline_template

Encapsulates the full template-reuse path:
  embed_query → search_templates (vector) → select_template (LLM judge)

Returns a clean {status, outline_tree, markdown, md_with_ids} dict.
The LLM judgment is done internally; callers only see found/not_found.
"""
...
async def search_outline_template(question: str) -> dict:
    """
    Search pre-built templates and let an internal LLM decide if any matches.

    Returns:
        {
            "status"      : "found" | "not_found",
            "outline_tree": dict,   # empty dict if not found
            "markdown"    : str,
            "md_with_ids" : str,
            "scene_name"  : str,    # matched template name (if found)
            "reason"      : str,    # LLM's reason (if not_found)
        }
    """
    ...
    if not candidates:
        logger.info("[Tool:search_outline_template] templates/ 目录为空")
        return _not_found("模板库为空，请走知识库生成")

    selected = await select_template(question, candidates)

    if not selected:
        logger.info("[Tool:search_outline_template] LLM 判断无匹配模板")
    ...
    logger.info(
        "[Tool:search_outline_template] 命中模板: %s (score=%.3f)",
        selected.get("scene_name", "?"),
        selected.get("_score", 0),
    )
    return {
        "status": "found",
