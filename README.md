...
| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | string | 知识库原始短 ID（如 `L4_002`）或新建节点 ID（如 `new_001`） |
| `uuid` | string | 原始业务 UUID，来自源 Excel；新建节点为空字符串 |
| `level` | int | 层级深度，1～4 为结构节点，5 为 query 节点 |
| `description` | string | L1～L3 为说明性文字；**L4 为章节导语（无占位符）**；**L5 的 description 直接作为查询参数执行数据过滤** |
| `condition` | string | 展示条件表达式，如 `${number("AEC覆盖用户数")>0}`；为空表示无条件展示 |
...
节点列表，每个节点一个对象。`uuid` 为原始业务 UUID，`id` 为短编号（供 LLM 引用），`level` 为整数 1～5：

```json
[
  {
    "uuid": "cc388999-2090-48f2-8109-32b0b73bb9b6",
    "id": "L1_001",
    "level": 1,
    "name": "超千兆升级(3000M升级)",
    "description": "牵引中国区客户超千兆升级，从家庭侧Wi-Fi7升级和局侧50GPON升级来驱动演进"
  },
  {
    "uuid": "2f424f18-5d50-45d8-b222-032d2148716d",
    "id": "L1_002",
    "level": 1,
    "name": "智能城域",
    "description": "包括城域BRAS扩容、CR扩容和BNG扩容等子场景"
  },
  {
    "uuid": "adf960eb-1234-5678-abcd-000000000001",
    "id": "L5_001",
    "level": 5,
    "name": "企业行业分布",
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `uuid` | string | 原始业务 UUID，来自源 Excel |
| `id` | string | 短编号（如 `L4_002`），FAISS 索引和大纲引用的主键 |
| `level` | int | 层级深度，1～5 整数 |
| `name` | string | 节点名称，向量检索的核心文本 |
| `description` | string | 节点说明，参与向量检索；L4 节点的 description 为章节导语（无占位符）|

> L4 节点的完整执行字段（`template`、`condition`、`condition_queries`、`dimensions` 等）存储在 `评估项.json`，不在 node.json 中。
